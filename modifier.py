...
def Process(user='Test'):
    """ set config data """
    main_path = "/home/johannes/Desktop/Projects/final_data/"
    ...
    all_training_files = [
        s.replace("/home/johannes/Desktop/Projects/final_data/" + user + "/Train data/", "") for s in
        all_training_files]
    ...
    for file__ in glob.glob( path + '/' + "*final_tuned_model.pkl"):
        ...
        for file in glob.glob(main_path + user + test_data_path + "*.csv"):
            ...
            best_model_.append(str(saving_path + 'FILE' + getFileName(file) + 'MODEL' + str(file__)[32:-4] + file[-4:]))
            best_model__.append(str(saving_path + 'FILE' + getFileName(file) + 'MODEL' + str(file__)[32:-4] + file[-4:]))

            testDF_copy.to_csv(saving_path + 'FILE' + getFileName(file) + 'MODEL' + str(file__)[32:-4] + file[-4:], index=False)
